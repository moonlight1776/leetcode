##第一题 TWO SUM
![Two Sum](https://upload-images.jianshu.io/upload_images/5220317-7c600d42c54317c4.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
输入：nums列表和目标值target，从nums中寻找相加=target的组合(唯一)
输出：对应的下标【从小到大】

```
第一次直接将list转dict，利用key来判断。value记录下标，但是发现nums中有重复
第二次发现耗时太长，又换了种写法。
```
##第二题 HAPPY NUMBER
输入：任意正数，判断时候满足如下规则；
...
2. 判断新数是否为1和n个0的组合
3.否则循环
4. 跳出条件：a.happy b.新数重复出现
![Happy Num](https://upload-images.jianshu.io/upload_images/5220317-795cd3f0d6bdf18c.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
 
参考资料：
https://www.jianshu.com/p/a4f8a6f9f541
【给出了一份很清晰的python构造dict的实现】
```
